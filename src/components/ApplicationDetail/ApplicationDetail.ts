...
import { formatInterviewType, formatInterviewStatus, formatTimelineEventType, formatDateTime } from "../../ui/format";
...
export function createApplicationDetail(documentRef: Document, options: ApplicationDetailOptions): HTMLElement {
  ...
  root.show = async (applicationId: string, tab: DetailTab = "overview"): Promise<void> => {
    ...
    panelFor("timeline").innerHTML = `<h4>时间线</h4><ol>${timeline.map((entry) => `<li>${esc(formatTimelineEventType(entry.type))}：${esc(entry.note)}</li>`).join("") || "<li>暂无事件</li>"}</ol>`;
    ...
    panelFor("interview").innerHTML = `
      ...
      <ul class="detail-interview-list">${interviews.length ? interviews.map((iv) => `
        <li data-interview-id="${esc(iv.id)}">
          <strong>第${esc(iv.round)}轮 · ${esc(formatInterviewType(iv.type))}</strong>
          <span>${esc(iv.title)}</span>
          <time datetime="${esc(iv.startsAt)}">${esc(formatDateTime(iv.startsAt, iv.timezone))}</time>
          <span class="badge">${esc(formatInterviewStatus(iv.status))}</span>
