...
import { formatInterviewStatus } from "../../ui/format";
...
/** 校验 IANA 时区是否可用于 Intl 格式化；未知/无效值（如手工存入的旧数据）回退 UTC，避免渲染崩溃。 */
...
function timezoneParts(date: Date, timezone: string): Record<string, number> {
  const formatter = new Intl.DateTimeFormat("en-CA", { timeZone: safeTimezone(timezone), year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" });
...
/** 全量 IANA 时区；Intl.supportedValuesOf 不可用/抛错/空时回退内置常用列表。UTC 是常用别名，部分运行环境的 supportedValuesOf 不含它，这里补齐。 */
...
export function createInterviewCalendar(documentRef: Document, options: InterviewCalendarOptions = {}): HTMLElement {
  ...
  setTimezoneSelect(form.elements.namedItem("timezone") as HTMLSelectElement, defaultTimezone);
  setTimezoneSelect(rescheduleForm.elements.namedItem("timezone") as HTMLSelectElement, defaultTimezone);
  ...
  const render = () => {
    ...
    root.querySelector(".in-app-reminders ul")!.innerHTML = reminders.map(({ interview, item }) => `<li>${escapeHtml(interview.title)}：${escapeHtml(formatReminderLocalTime(interview.startsAt, safeTimezone(interview.timezone), item))}（${escapeHtml(interview.timezone)}）</li>`).join("") || "<li>暂无应用内提醒</li>";
