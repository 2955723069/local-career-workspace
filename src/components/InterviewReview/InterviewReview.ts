...
export function createInterviewReview(documentRef: Document, options: InterviewReviewOptions = {}): HTMLElement {
  const uid = `ir-${++instanceCount}`;
  const root = documentRef.createElement("section"); root.className = "interview-review"; root.dataset.component = "interview-review"; root.setAttribute("aria-labelledby", `${uid}-interview-review-title`);
  root.innerHTML = `<div class="section-heading"><div><p class="section-label">记录</p><h2 id="${uid}-interview-review-title">面试复盘</h2></div><span data-review-status role="status" aria-live="polite" aria-atomic="true">请选择面试</span></div><form data-form="review" aria-labelledby="${uid}-interview-review-title"><label for="${uid}-review-rating">评分（0 至 5）<input id="${uid}-review-rating" data-review-field="overallRating" type="number" min="0" max="5" step="1" /></label><fieldset class="interview-review__questions"><legend>面试问题记录</legend><div class="interview-review__question-list" data-question-list></div><button type="button" data-action="add-question">添加问题</button></fieldset><label for="${uid}-review-good-answers">优势回答<textarea id="${uid}-review-good-answers" data-review-field="goodAnswers"></textarea></label><label for="${uid}-review-weak-answers">薄弱回答<textarea id="${uid}-review-weak-answers" data-review-field="weakAnswers"></textarea></label><label for="${uid}-review-observed-signals">观察信号<textarea id="${uid}-review-observed-signals" data-review-field="observedSignals"></textarea></label><label for="${uid}-review-salary">薪资讨论<textarea id="${uid}-review-salary" data-review-field="salaryDiscussion"></textarea></label><label for="${uid}-review-next-step">下一步<textarea id="${uid}-review-next-step" data-review-field="nextStep"></textarea></label><label for="${uid}-review-private-note">私密备注<textarea id="${uid}-review-private-note" data-review-field="privateNote"></textarea></label><div class="interview-review__actions"><button type="submit">保存复盘</button><button type="button" data-action="cancel-review">取消编辑</button></div></form>`;
